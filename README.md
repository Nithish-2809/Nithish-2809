...
I'm a second-year Computer Science undergraduate at **Mahatma Gandhi Institute of Technology (MGIT), Hyderabad**, maintaining a **9.3/10 CGPA**. I build full-stack and AI-integrated products end to end — from offline desktop applications with custom OCR pipelines to real-time collaboration platforms handling live socket events at scale.

- 🎓 B.Tech CSE, MGIT Hyderabad (2024 – 2028) — CGPA 9.3/10
- 🏗️ Shipped three production-grade projects: an offline AI search engine, a real-time project management platform, and a geospatial marketplace
- 🥈 Runner-up, 24-Hour Hackathon at KL University (120+ teams) · Top 7, JNTU Brainstorm
- 🧮 440+ DSA problems solved across LeetCode and GeeksforGeeks
- 📜 Certified — Postman API Fundamentals · Data Science with NumPy & Pandas (CampusX)
...
### ElectricSearch
**Offline-first desktop search engine** built on Electron.js — indexes 1,000+ images via OCR and returns full-text and semantic search results without any cloud dependency.

- Multi-threaded OCR pipeline using Node.js Worker Threads — **+56% throughput**
- Offline semantic search with `Xenova/bge-small-en-v1.5` (384-dim embeddings, cosine similarity), 40–60ms retrieval
- SQLite FTS5 indexing delivering **8–12ms** full-text search
- Secure architecture: 25+ IPC APIs, preload scripts, context isolation

`Electron.js` `React.js` `Node.js` `SQLite FTS5` `Tesseract OCR` `Transformers`

**[Repository →](https://github.com/Nithish-2809)** &nbsp;·&nbsp; **[Live Demo →](https://github.com/Nithish-2809)**

</td>
<td width="50%" valign="top">

### TeamFlow
**Trello-inspired real-time collaboration platform** exposing 40+ REST APIs across authentication, boards, tasks, invitations, and team messaging.

- Real-time board sync via Socket.IO — 10+ synchronized live events
- Transaction-safe MongoDB backend with RBAC and cascade deletion
- Modular React frontend using custom hooks, Zustand, and drag-and-drop
- Async email delivery via BullMQ + Redis background workers

`React.js` `Node.js` `MongoDB` `Socket.IO` `JWT` `Redis` `BullMQ`
...
### LocalLynk
**Hyperlocal MERN marketplace** enabling location-based buying and selling with geospatial search and secure payments.

- MongoDB 2dsphere indexes for radius-based location discovery
- JWT + Google OAuth authentication with protected routes
- Stripe-integrated secure payments
- 15+ REST APIs covering the full order lifecycle

`React.js` `Node.js` `Express.js` `MongoDB` `Stripe API` `JWT`
...
- 🥈 Runner-up — 24-Hour Hackathon, KL University (120+ teams)
- 🏅 Top 7 — JNTU Brainstorm Hackathon
- 🧮 440+ DSA problems solved (LeetCode + GFG)
- 📜 Postman API Fundamentals — Certified
- 📜 Data Science Master Program (NumPy & Pandas) — CampusX
...
<img src="https://raw.githubusercontent.com/Nithish-2809/Nithish-2809/main/assets/contribution-calendar.svg" width="100%" alt="Nithish-2809's contribution calendar"/>
